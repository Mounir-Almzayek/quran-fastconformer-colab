...
def _stage_trainer(stage_dir: Path, stage: Mapping[str, Any], config: Mapping[str, Any]) -> tuple[Any, Any, Any]:
    """Create one trainer with best-validation and frequent Drive recovery checkpoints."""
    pl, _, _ = import_nemo()
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[best_checkpoint, recovery_checkpoint],
...
def run(config_path: str | Path, manifest_path: str | Path) -> dict[str, Any]:
    ...
    for index, stage in enumerate(config["training"]["stages"], start=1):
        ...
        trainer, best_checkpoint, recovery_checkpoint = _stage_trainer(stage_dir, stage, config)
